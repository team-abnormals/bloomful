...
public class WisteriaBoatEntity extends BoatEntity {
    ...
    public WisteriaBoatEntity(EntityType<? extends BoatEntity> type, World world) {
        super(type, world);
    ...
    protected void registerData() {
        ...
        dataManager.register(BOAT_TYPE, Type.WISTERIA.ordinal());
        ...
        if (!isPassenger()) {
            if (onGroundIn) {
                if (fallDistance > 3.0F) {
                    if (status != WisteriaBoatEntity.Status.ON_LAND) {
                        fallDistance = 0.0F;
                        return;
                    }
